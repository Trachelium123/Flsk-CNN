...
# dwt
coeff_train = pywt.dwt(X_train, 'db3', mode='symmetric')
X_train = coeff_train[0]
coeff_val = pywt.dwt(X_val, 'db3', mode='symmetric')
X_val = coeff_val[0]
...
# with SummaryWriter(comment='CNN_Model')as w:
#     w.add_graph(cnn_model, (X_train,))
...
# 保存模型
torch.save(cnn_model, 'cnn_model2.pth')
# 加载模型
# cnn_model = torch.load('data/cnn_model2.pth')
