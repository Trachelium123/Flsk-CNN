...
from python.meiyongde.KSVD import KSVD
from python.meiyongde.maxmin import maxminnorm

if __name__ == '__main__':

    # os.environ["PATH"] += os.pathsep + 'C:/Program Files/Java/jre7/bin'
    # 载入数据
    data1 = []
    label = []
    ...
    # # dwt变换
    # coeff = pywt.dwt(data1, 'db10', mode='symmetric')
    ...
    # 数据拆分
    X_train, X_test, y_train, y_test = sklearn.model_selection.train_test_split(data, label, test_size=0.25, shuffle=True)

    # nn
    nn = PMMLPipeline([("classifier",
                        sk_nn.MLPClassifier(activation='tanh', solver='adam', alpha=0.0001, learning_rate='adaptive',
